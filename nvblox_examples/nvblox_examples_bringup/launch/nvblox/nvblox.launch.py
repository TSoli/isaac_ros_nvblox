...
def generate_launch_description():
    ...
    tello_config = os.path.join(specialization_dir, "tello.yaml")
    ...
    setup_for_tello = IfCondition(
        LaunchConfiguration("setup_for_tello", default="False")
    )
    ...
    group_action = GroupAction(
        [
            ...
            SetParametersFromFile(tello_config, condition=setup_for_tello),
            SetParameter(
                name="global_frame",
            ...
            SetRemap(
                src=["pose"], dst=["/zed2/zed_node/pose"], condition=setup_for_zed
            ),
            ...
            SetRemap(
                src=["pointcloud"], dst=["/point_cloud"], condition=setup_for_isaac_sim
            ),
            # Remappings for Tello
            SetRemap(src=["color/image"], dst=["/camera/image"]),
            SetRemap(src=["depth/image"], dst=["/camera/depth"]),
            SetRemap(
                src=["color/camera_info"],
                dst=["/camera_info"],
            ),
