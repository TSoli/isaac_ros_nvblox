...
def generate_launch_description():
    ...
    group_action = GroupAction(
        [
            ...
            SetParameter(
                name="use_sim_time", value=True, condition=setup_for_isaac_sim
            ),
            SetParameter(
                name="denoise_input_images", value=True, condition=setup_for_isaac_sim
            ),
            SetParameter(
                name="rectified_images", value=False, condition=setup_for_isaac_sim
            ),
            SetParameter(
                name="base_frame", value="base_link", condition=setup_for_isaac_sim
            ),
            SetParameter(
                name="input_base_frame",
                value="base_link",
                condition=setup_for_isaac_sim,
            ),
            SetParameter(
                name="input_left_camera_frame",
                value="camera_left_ROS_frame",
                condition=setup_for_isaac_sim,
            ),
            SetParameter(
                name="input_right_camera_frame",
                value="camera_right_ROS_frame",
                condition=setup_for_isaac_sim,
            ),
            # Parameters for Realsense
            SetParameter(
                name="enable_rectified_pose", value=True, condition=setup_for_realsense
            ),
            SetParameter(
                name="denoise_input_images", value=False, condition=setup_for_realsense
            ),
            SetParameter(
                name="rectified_images", value=True, condition=setup_for_realsense
            ),
            SetParameter(
                name="base_frame", value="camera_link", condition=setup_for_realsense
            ),
            # Remappings for Isaac Sim
            SetRemap(
                src=["/stereo_camera/left/camera_info"],
                dst=["/front/stereo_camera/left/camera_info"],
                condition=setup_for_isaac_sim,
            ),
            SetRemap(
                src=["/stereo_camera/right/camera_info"],
                dst=["/front/stereo_camera/right/camera_info"],
                condition=setup_for_isaac_sim,
            ),
            SetRemap(
                src=["/stereo_camera/left/image"],
                dst=["/front/stereo_camera/left/rgb"],
                condition=setup_for_isaac_sim,
            ),
            SetRemap(
                src=["/stereo_camera/right/image"],
            ...
            SetParameter(
                name="input_parent_frame_id",
                value=odometry_frame_before_flattening_name,
            ),
            SetParameter(
                name="input_child_frame_id",
                value="base_link",
                condition=setup_for_isaac_sim,
            ),
            SetParameter(
